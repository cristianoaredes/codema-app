  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Resoluções</h1>
          <p className="text-muted-foreground">
            Controle de resoluções do CODEMA - criação, votação e publicação
          </p>
        </div>
        {canEdit && (
          <Dialog open={showResolucaoForm} onOpenChange={setShowResolucaoForm}>
            ...
            <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle>
                  {selectedResolucao ? 'Editar Resolução' : 'Nova Resolução'}
                </DialogTitle>
              </DialogHeader>
              <ResolucaoForm
                resolucao={selectedResolucao}
                onClose={() => {
                  setShowResolucaoForm(false);
        ...
        <Dialog open={showVotingSystem} onOpenChange={setShowVotingSystem}>
          <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            {votingResolucaoId && (
              <VotingSystem
                resolucaoId={votingResolucaoId}
                canVote={canVote}
                onClose={() => {
                  setShowVotingSystem(false);
                  setVotingResolucaoId(null);
                }}
              />
            )}
          </DialogContent>
          ...
          <TabsContent value="tipo" className="space-y-4">
            <div className="grid gap-4 md:grid-cols-4">
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">{stats.normativas}</div>
                </CardContent>
              </Card>
              <Card>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                  <CardTitle className="text-sm font-medium">Deliberativas</CardTitle>
                  <Vote className="h-4 w-4 text-muted-foreground" />
                </CardHeader>
                <CardContent>
                  <div className="text-2xl font-bold">{stats.deliberativas}</div>
                </CardContent>
              </Card>
              <Card>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                  <CardTitle className="text-sm font-medium">Administrativas</CardTitle>
                  <Users className="h-4 w-4 text-muted-foreground" />
                </CardHeader>
                <CardContent>
                  <div className="text-2xl font-bold">{stats.administrativas}</div>
                </CardContent>
              </Card>
            </div>
          </TabsContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Pesquise e filtre as resoluções por diferentes critérios
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Lista de Resoluções</CardTitle>
          <CardDescription>
            {resolucoes.length} resolução(ões) encontrada(s)
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
              ...
              {resolucoes.map((resolucao) => (
                <TableRow key={resolucao.id}>
                  ...
                  <TableCell className="max-w-[300px]">
                    <div>
                      <p className="font-medium truncate">{resolucao.titulo}</p>
                      <p className="text-sm text-muted-foreground truncate">{resolucao.ementa}</p>
                    </div>
                  </TableCell>
                    ...
                    {resolucao.resultado_votacao && (
                      <div className="text-sm">
                        <div className="flex gap-1">
                          <span className="text-green-600">✓{resolucao.votos_favor}</span>
                          <span className="text-red-600">✗{resolucao.votos_contra}</span>
                          <span className="text-gray-500">-{resolucao.abstencoes}</span>
                        </div>
                        <div className="text-xs text-muted-foreground">
                          {resolucao.quorum_presente} presentes
                        </div>
                      </div>
                    )}
                  </TableCell>
                  <TableCell>
                    <div className="text-sm">
                      {resolucao.data_publicacao && (
                        <div>Pub: {format(new Date(resolucao.data_publicacao), "dd/MM/yy", { locale: ptBR })}</div>
                      )}
                      {resolucao.data_votacao && !resolucao.data_publicacao && (
                        <div>Vot: {format(new Date(resolucao.data_votacao), "dd/MM/yy", { locale: ptBR })}</div>
                      )}
                      {!resolucao.data_votacao && (
                        <div>Criada: {format(new Date(resolucao.created_at), "dd/MM/yy", { locale: ptBR })}</div>
                      )}
                    </div>
                  </TableCell>
                      ...
                      {canEdit && (
                        <Button
                          ...
                          onClick={() => {
                            ...
                            setShowResolucaoForm(true);
                          }}
                        >
                          <Edit2 className="w-4 h-4" />
                        </Button>
                      )}
                      {(canVote || canEdit) && resolucao.status === 'em_votacao' && (
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => {
                            setVotingResolucaoId(resolucao.id);
                            setShowVotingSystem(true);
                          }}
                        >
                          <Vote className="w-4 h-4" />
                        </Button>
                      )}
                      {resolucao.pdf_gerado && (
                        <Button variant="outline" size="sm">
                    ...
                    <div className="flex flex-col items-center gap-2">
                      ...
                      {canEdit && (
                        <p className="text-sm text-muted-foreground">
                          Clique em "Nova Resolução" para criar a primeira resolução
                        </p>
                      )}
                    </div>
                  </TableCell>
