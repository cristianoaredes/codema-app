...
export default function GestaoProtocolos() {
  ...
  useEffect(() => {
    if (tipoSelecionado) { consultarProximo(); }
  ...
  const consultarProtocolo = () => {
    if (!validarFormatoProtocolo(numeroConsulta)) {
      notification.show({ title: 'Formato Inválido', message: 'Formato do protocolo deve ser: TIPO-000/YYYY', variant: 'error' });
      return;
    }
    const info = extrairInfoProtocolo(numeroConsulta);
    if (info) {
      setInfoProtocolo(info);
      notification.show({ title: 'Protocolo Encontrado', message: `Protocolo ${numeroConsulta} consultado com sucesso`, variant: 'success' });
    } else {
      notification.show({ title: 'Protocolo Inválido', message: 'Não foi possível extrair informações do protocolo', variant: 'error' });
    }
  };

  const resetarSequencia = async (tipo: TipoProtocolo) => {
    try {
      // TODO: Implementar resetarSequencia via API/Supabase quando necessário
  ...
  const getTiposDisponiveis = () => [2024, 2025, 2026].map(ano => ({ value: ano, label: ano.toString() }));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        }).length} description="Tipos utilizados" icon={Calendar} color="purple" />
      </div>
      ...
      <Tabs defaultValue="estatisticas" className="w-full">
        ...
        <TabsContent value="estatisticas" className="space-y-4">
          <Card>
            <CardHeader><CardTitle className="flex items-center"><BarChart3 className="w-5 h-5 mr-2" />Estatísticas por Tipo</CardTitle></CardHeader>
            <CardContent>
                    ...
                    {estatisticas.map((stat) => (
                      <div key={stat.tipo} className="border rounded-lg p-4">
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <Badge variant={stat.total_gerados > 0 ? "default" : "secondary"}>{stat.tipo}</Badge>
                        </div>
                        <div className="text-2xl font-bold text-blue-600 mb-1">{stat.total_gerados}</div>
                        <p className="text-sm text-muted-foreground">{TIPOS_PROTOCOLO_INFO[stat.tipo].descricao}</p>
                        <div className="mt-2 text-xs text-muted-foreground">Última atualização: {new Date(stat.ultima_atualizacao).toLocaleDateString('pt-BR')}</div>
                      </div>
                    ))}
                  </div>
                )}
        ...
        <TabsContent value="gerador" className="space-y-4">
          <Card>
            <CardHeader><CardTitle className="flex items-center"><Plus className="w-5 h-5 mr-2" />Gerador de Protocolos</CardTitle></CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <Select value={tipoSelecionado} onValueChange={(value) => setTipoSelecionado(value as TipoProtocolo)}>
                    ...
                    <SelectContent>
                      {Object.entries(TIPOS_PROTOCOLO_INFO).map(([tipo, info]) => (
                        <SelectItem key={tipo} value={tipo}>{info.nome} ({tipo})</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </div>
                <div>
                  ...
                  <Input value={proximoNumero} disabled className="bg-muted" />
                </div>
              </div>
              <Alert>
                <Info className="h-4 w-4" />
                <AlertDescription><strong>{TIPOS_PROTOCOLO_INFO[tipoSelecionado].nome}:</strong> {TIPOS_PROTOCOLO_INFO[tipoSelecionado].descricao}</AlertDescription>
              </Alert>
              <div className="flex justify-end">
                <Button onClick={gerarNovoProtocolo} disabled={carregandoGerador} className="min-w-40">
                  {carregandoGerador ? <RefreshCw className="w-4 h-4 mr-2 animate-spin" /> : <Plus className="w-4 h-4 mr-2" />}
                  Gerar Protocolo
                </Button>
              </div>
            </CardContent>
        ...
        <TabsContent value="consulta" className="space-y-4">
          <Card>
            <CardHeader><CardTitle className="flex items-center"><Search className="w-5 h-5 mr-2" />Consulta de Protocolos</CardTitle></CardHeader>
            <CardContent className="space-y-4">
              <div className="flex space-x-4">
                <div className="flex-1">
                  <Label htmlFor="numero">Número do Protocolo</Label>
                  <Input id="numero" value={numeroConsulta} onChange={(e) => setNumeroConsulta(e.target.value)} placeholder="Ex: PROC-001/2024" className="font-mono" />
                </div>
                <div className="flex items-end">
                  <Button onClick={consultarProtocolo}><Search className="w-4 h-4 mr-2" />Consultar</Button>
                </div>
              </div>
              {infoProtocolo && (
                <Alert>
              ...
              <div className="border-t pt-4">
                ...
                <p className="text-sm text-muted-foreground">TIPO-000/YYYY onde TIPO é o código do tipo de protocolo, 000 é o número sequencial com 3 dígitos e YYYY é o ano.</p>
              </div>
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="configuracoes" className="space-y-4">
          <Card>
            <CardHeader><CardTitle className="flex items-center"><Settings className="w-5 h-5 mr-2" />Configurações</CardTitle></CardHeader>
            <CardContent className="space-y-4">
              <Alert>
                <AlertCircle className="h-4 w-4" />
                <AlertDescription><strong>Atenção:</strong> As operações abaixo devem ser realizadas com cuidado, pois podem afetar a numeração dos protocolos.</AlertDescription>
              </Alert>
              <div className="space-y-4">
                <h4 className="font-medium">Resetar Sequências</h4>
                <p className="text-sm text-muted-foreground">Resetar uma sequência fará com que a numeração volte a começar do 001.</p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {Object.entries(TIPOS_PROTOCOLO_INFO).map(([tipo, info]) => (
                    <div key={tipo} className="border rounded-lg p-4">
                      <div className="flex items-center justify-between">
                        ...
                        <Button variant="outline" size="sm" onClick={() => resetarSequencia(tipo as TipoProtocolo)}>Resetar</Button>
                      </div>
                    </div>
                  ))}
                </div>
